...
local function constructNew_frmMazeOptions()
    ...
    obj.btnCancel1:setMargins({left=4, right=4});
		...
		self.tabName.text = lang('scene.generator.maze');
		...
		local function buildMaze(grid, x, y, stack, stackSize)
			-- showMessage("x: " .. x .. " y: " .. y .. " size: " .. stackSize);

			grid[x][y].status = "visited";

			local paths = findPaths(grid, x, y);
			-- showMessage(paths);

			if paths == 0 then
				-- deadend go to next in the list

				stackSize = stackSize - 1;
				if stackSize > 0 then
					buildMaze(grid, stack[stackSize].x, stack[stackSize].y, stack, stackSize)
				end;
			else
				stackSize = stackSize + 1;
				stack[stackSize] = grid[x][y];

				x, y = selectPath(grid, x, y, paths);
				buildMaze(grid, x, y, stack, stackSize);
			end;
		end;
		...
		function self:processarOK()	
			...
			-- Criando labirinto
			buildMaze(map,math.random(1,horzSquare), math.random(1,vertSquare), stack, 0);
...
function newfrmMazeOptions()
    local retObj = nil;
    __o_rrpgObjs.beginObjectsLoading();

    __o_Utils.tryFinally(
      function()
        retObj = constructNew_frmMazeOptions();
      end,
      function()
...
local _frmMazeOptions = {
    newEditor = newfrmMazeOptions, 
    new = newfrmMazeOptions, 
    name = "frmMazeOptions", 
    dataType = "", 
    formType = "undefined", 
...
frmMazeOptions = _frmMazeOptions;
Firecast.registrarForm(_frmMazeOptions);

return _frmMazeOptions;
