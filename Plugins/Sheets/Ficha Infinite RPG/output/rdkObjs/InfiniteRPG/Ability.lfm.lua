...
local function constructNew_frmAbility()
    ...
    obj:setMargins({top=5,bottom=5,left=5,right=5});
		...
		local function showPopup()
			local pop = self:findControlByName("popAbility");
...
function newfrmAbility()
    local retObj = nil;
    __o_rrpgObjs.beginObjectsLoading();

    __o_Utils.tryFinally(
      function()
        retObj = constructNew_frmAbility();
      end,
      function()
...
frmAbility = _frmAbility;
Firecast.registrarForm(_frmAbility);
